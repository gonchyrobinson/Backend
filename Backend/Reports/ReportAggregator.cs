...
namespace Backend.Reports
{
    public class ReportAggregator : IReporteDataAggregator<int, ReportData>
    {
        ...
        public async Task<ReportData> GetDataAsync(int id)
        {
            ...
            if (pasantia == null)
                throw new Exception($"No se encontró la pasantía con id {id}");
            ...
            if (estudiante == null)
                throw new Exception($"No se encontró el estudiante con id {idEstudiante}");
            ...
            if (convenio == null)
                throw new Exception($"No se encontró el convenio con id {idConvenio}");

            // Obtener la empresa
            var idEmpresa = convenio.IdEmpresa ?? throw new Exception($"El convenio {idConvenio} no tiene IdEmpresa asociado");
