...
namespace Backend.DTOValidations
{
    public class PasantiaDtoValidator : BaseValidator
    {
        ...
        public static IEnumerable<ValidationResult> ValidatePasantiaWithDni(
            ...
            bool isCreate = false)
        {
            // Validaciones requeridas - usando DNI en lugar de ID
            if (IsNullOrWhiteSpace(dniEstudiante))
                yield return CreateValidationResult("El DNI del estudiante es obligatorio.", nameof(dniEstudiante));

            // Validar formato de DNI si se proporciona
            if (!string.IsNullOrEmpty(dniEstudiante) && !CommonValidations.EsDniValido(dniEstudiante))
                yield return CreateValidationResult("El DNI del estudiante debe ser válido.", nameof(dniEstudiante));
